    ...
    while True:
        ...
        if choice in ['y', 'yes']:
            ...
            try:
                ...
                success = run_insert_section(target_file, is_fallback=True)
            except ImportError:
                from insert_section import run_insert_section
                success = run_insert_section(target_file, is_fallback=True)
            
            if success:
                print("✅ Section generated successfully — continuing with normal generation...")
                try:
                    from .insert_section import run_insert_section
                    run_insert_section(target_file, is_fallback=False)
                except ImportError:
                    from insert_section import run_insert_section
                    run_insert_section(target_file, is_fallback=False)
                return True
